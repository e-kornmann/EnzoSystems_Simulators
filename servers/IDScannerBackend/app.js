try {
  ...
  app.get(`/${process.env.API_BASE_PATH}/v${process.env.API_VERSION}/health`, function (req, res) {
    res.status(httpStatus.StatusCodes.OK).json({ info: 'The ID Scanner back-end server is healthy!' });
  });
  ...
  console.info(`The id scanner back-end server started and is listening on port: ${process.env.PORT}`);
} catch (error) {
